    ...
    private void addClass(HttpServletRequest request, HttpServletResponse response) throws IOException {
        try {
            ...
            if (startDate.after(endDate)) {
                AuthUtil.storeErrorMessage(request, "Ngày bắt đầu phải trước ngày kết thúc!");
                response.sendRedirect(request.getContextPath() + "/admin/classes");
                return;
            }
            ...
            AuthUtil.storeErrorMessage(request, "Dữ liệu số không hợp lệ!");
            response.sendRedirect(request.getContextPath() + "/admin/classes");
        } catch (IllegalArgumentException e) {
            AuthUtil.storeErrorMessage(request, "Định dạng ngày không hợp lệ!");
            response.sendRedirect(request.getContextPath() + "/admin/classes");
        } catch (Exception e) {
            e.printStackTrace();
