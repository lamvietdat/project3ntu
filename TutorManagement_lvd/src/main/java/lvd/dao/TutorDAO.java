 ...
 // Phương thức cập nhật thông tin gia sư
    public boolean updateTutor(Tutor tutor) {
        ...
        try {
            ...
            if (tutor == null || tutor.getTutorId() <= 0) {
                return false;
            }
            
            // Kiểm tra gia sư có tồn tại không
            Tutor existingTutor = getTutorById(tutor.getTutorId());
            if (existingTutor == null) {
                return false;
            }
            
            String sql = "UPDATE tutors SET qualification = ?, experience = ?, hourly_rate = ?, " +
                         "bio = ?, status = ? WHERE tutor_id = ?";
            stmt = conn.prepareStatement(sql);
            ...
            // Nếu cập nhật thành công và user được thiết lập
            if (success && tutor.getUser() != null) {
                // Cập nhật thông tin user của gia sư
                UserDAO userDAO = new UserDAO();
                userDAO.updateUser(tutor.getUser());
            }
        } catch (SQLException e) {
            e.printStackTrace();
        } finally {
